...
CREATE TABLE train_transaction_features AS
SELECT *
...
CREATE TABLE train_transaction_time_features AS
SELECT
    "TransactionID",
    "TransactionDT",
    "TransactionAmt",
...
FROM train_transaction_features;

-- Amount/Average Ratio
ALTER TABLE train_transaction_time_features
ADD COLUMN amt_vs_avg_ratio float8;

UPDATE train_transaction_time_features
...
FROM step2
WHERE train_transaction_time_features."TransactionID" = step2."TransactionID";
...
ON train_transaction_time_features (
    "uid1",
    "DeviceInfo",
    "DeviceType",
    "TransactionDT"
...
UPDATE train_transaction_time_features t1
SET is_new_device_uid1 =
    CASE WHEN EXISTS(
        SELECT 1 FROM train_transaction_time_features t2
...
--   AND table_name = 'train_transaction_features';


CREATE TABLE train_final_features AS
...
FROM train_transaction_time_features t
LEFT JOIN train_transaction_features AS f ON t."TransactionID" = f."TransactionID"
