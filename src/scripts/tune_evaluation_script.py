...
def main():
    logger.info("Starting STANDALONE Evaluation mode.")

    if not LGBM_MODEL_PATH.exists():
        logger.error("LGBM model not found! Please run the full training pipeline first.")
        sys.exit(1)
    ...
    try:
        ...
        logger.error("run_id not found in inference meta! Cannot log to MLflow.")
        sys.exit(1)

    logger.info("Loading trained LightGBM model from disk...")
    model_lgbm = lgb.Booster(model_file=str(LGBM_MODEL_PATH))

    logger.info("Loading cached parquet datasets...")
    ...
    logger.info("Passing data to the evaluation pipeline...")
    evaluation_pipeline(model_lgbm, X_train, y_train, X_val, y_val, X_test, y_test, run_id, shap=False)

    logger.info("Standalone Evaluation finished successfully!")
