...
First of all, the endpoint is protected by an API-key dependency. Pydantic validates the incoming transaction schema before it reaches the inference service.
...
However, the primary focus of this project was not to obtain a high score, but to build a complete, production-oriented MLOps pipeline.